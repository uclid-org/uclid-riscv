...
def parse_elf(elf_path):
    ...
    for i, (t, t_entries) in enumerate(tests.items()):
        ...
        last_step = len(t_entries) + max(len(fail_insts), len(pass_insts))
        s = textwrap.dedent(f"""\
...
{textwrap.indent(init_block, ' ' * 20)}
                ...
                invariant exits_ecall: ((step == {last_step - 1}) ==> (cpu_0.exception.cause == X_ECALL && cpu_0.regfile[registers.a0] == 0bv32 && cpu_0.exception.epc == {expected_epc}));
